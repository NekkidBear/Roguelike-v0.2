...
def render_bar(panel, x, y, total_width, name, value, maximium, bar_color, back_color):
    bar_width = int(float(value)/maximium*total_width)
    ...
    libtcod.console_print_ex(panel, int(x + total_width / 2), y, libtcod.BKGND_NONE, libtcod.CENTER, '{0}: {1}/{2}'.format(name, value, maximium))
...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height,
    ...
    if game_state == GameStates.SHOW_INVENTORY:
        inventory_menu(con, 'Press the key next to an item to use it, or Esc to cance. \n', player.inventory, 50, screen_width, screen_height)
